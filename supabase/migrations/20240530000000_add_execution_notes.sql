-- Migration: Add Execution Notes to Market Tasks

-- 1. Add execution_notes column to market_tasks table
alter table public.market_tasks
  add column if not exists execution_notes text;

-- 2. Update get_market_board RPC to include execution_notes
...
begin
  ...
  local_tasks as (
    select
      ...
      mt.description,
      mt.task_type,
      null::uuid as origin_template_task_id, -- Cast to UUID to match template_based_tasks
      0 as weight,
