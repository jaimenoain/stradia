...
FOR ALL
USING (id::uuid = (select auth.jwt() ->> 'tenant_id')::uuid)
WITH CHECK (id::uuid = (select auth.jwt() ->> 'tenant_id')::uuid);

-- User Policies
-- Users can only see/modify users in their own tenant
CREATE POLICY "tenant_isolation" ON "User"
FOR ALL
USING (tenant_id::uuid = (select auth.jwt() ->> 'tenant_id')::uuid)
WITH CHECK (tenant_id::uuid = (select auth.jwt() ->> 'tenant_id')::uuid);
...
USING (
  exists (
    ...
    and tenant_id::uuid = (select auth.jwt() ->> 'tenant_id')::uuid
  )
)
WITH CHECK (
  exists (
...
FOR ALL
USING (
  tenant_id::uuid = (select auth.jwt() ->> 'tenant_id')::uuid
  AND
  (
